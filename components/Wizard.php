...
class Wizard extends ComponentBase
{
  ...
  public function formsValidate($validatePrevSteps = false)
  {
    ...
    if (request()->ajax()) {
      ...
      $resp = ($validatePrevSteps ? $this->stepValidate() : (isset($wizardSession['validations']) ? $wizardSession['validations'] : []));
      ...
      if (isset($this->steps[$stepPos]['forms'][$method]['validation']) || isset($this->steps[$stepPos]['forms'][$method]['extra_validation'])) {
        ...
        if (isset($formStep['extra_validation'])) {
          $validator->after(function ($validator) use ($formStep, &$resp, $requestData) {
            $result = $formStep['extra_validation']($validator, array_merge($requestData, session()->get('wizard_steps', [])), $resp);
      ...
      session(['wizard_steps' => array_merge(session()->get('wizard_steps', []), ['stepCurrent' => ($stepPos + 1)])]);
  ...
  public function stepSessionClear($currentStepPos)
  {
    ...
    $newSession = array_merge(Arr::except(session()->get('wizard_steps', []), $dataToClear), ['stepCurrent' => $currentStepPos]);
